...
#region ERROR CHECKS  ------------------------------------------------------------------------------------------------------------
    ...
    wall = Element(0.5, 0.7, [20, 0.01],  [wallmt3, wallmt3], 0, 296, False, 'common_brick_wall_with_plaster')
    roof = Element(0.5, 0.7, [40, 0.025], [roofmtl, roofmtl], 0, 296, True, 'tile')
...
param_values = saltelli.sample(problem, 1024) #14,500
...
#Y = np.loadtxt(base_path + "txtexport\\izmir_morris.txt")
                        ...
                        #_izmir_morris_9-29
np.savetxt(base_path + "txtexport\\sobol-urban_characteristics-1-18-NUMPY.txt", Y)
...
with open(base_path + 'txtexport\\sobol-urban_characteristics-1-18.txt', 'w') as f:
    for line in lines:
        f.write(line)
        f.write('\n')
        f.write('------------')
...
df.to_csv(base_path + "csvexport\\sobol-urban_characteristics-1-18.csv")