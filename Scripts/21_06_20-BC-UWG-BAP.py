...
def custom_uwg(directory):
    ...
    concrete = Material(0.5, 1000000 * 2.086, 'wood')

    # elements
    wall = Element(0.22, 0.92, [0.1, 0.1, 0.0127], [concrete, insulation, gypsum], 0, 293, False, 'wood_frame_wall')
    
    roof = Element(0.22, 0.92, [0.1, 0.1, 0.0127], [concrete, insulation, gypsum], 0, 293, True, 'wood_frame_roof')
    
    mass = Element(0.20, 0.9, [0.5, 0.5], [concrete, concrete], 0, 293, True, 'wood_floor')
    ...
    model = UWG.from_param_args(
        epw_path=epw_path, bldheight=24.0, blddensity=0.26, vertohor=1.21, zone='5A',
        treecover=0.14, grasscover=0.16, bld=bld, ref_bem_vector=ref_bem_vector,
        ref_sch_vector=ref_sch_vector, month=1, day=1, sensanth=10, nday=365, dtsim=200, vegroof = 0)
