...
# sample
param_values = saltelli.sample(problem, 1024) #2653
...
df.to_csv(base_path + "csvexport\\sobol-building_characteristics-2-1.csv")


lines = [str(Si_Temp), str(Si_CDD), str(Si_HDD)]
with open(base_path + 'txtexport\\sobol-building_characteristics-2-1.txt', 'w') as f:
    for line in lines:
        f.write(line)
        f.write('\n')
        f.write('------------')
