...
intermediate_epw_path = "data/simulation2h.epw"
...
def evaluate_epw():
    ...
    for params in param_values:
        try:
            ...
            for h in range(0,7):
                for b in range(0, 24):
                    hourly_temperature = pd_epw_sens['temp_air'].values[(24 * h) + b + 793]
                    hourly_base_temperature = base_epw['temp_air'].values[(24 * h) + b + 793]
...
df.to_csv(base_path + "\\csvexport\\sobol-hourly-5-29-uc-w-S1.csv")
...
df = pd.DataFrame(data) 
df.to_csv(base_path + "\\csvexport\\sobol-hourly-5-29-uc-w-ST.csv")
...
with open(base_path + '\\txtexport\\sobol-hourly-5-29-uc-w.txt', 'w') as f:
    for line in lines:
        f.write(line)
        f.write('\n')
        f.write('------------')
...
df = pd.DataFrame(data) 
df.to_csv(base_path + "\\csvexport\\sobol-hourly-5-29-uc-w.csv")