...
df.to_excel(base_path + "\\csvexport\\sobol-hourly-6-16-uc-w-S1.xlsx")
...
df = pd.DataFrame(data)
df.to_excel(base_path + "\\csvexport\\sobol-hourly-6-16-uc-w-ST.xlsx")
...
with open(base_path + '\\txtexport\\sobol-hourly-6-16-uc-w.txt', 'w') as f:
    for line in lines:
        f.write(line)
        f.write('\n')
        f.write('------------')
...
df = pd.DataFrame(data)
df.to_excel(base_path + "\\csvexport\\sobol-hourly-6-16-uc-w.xlsx")
