...
#region ERROR CHECKS  ------------------------------------------------------------------------------------------------------------
    ...
    model = UWG.from_param_args(
        ...
        ref_sch_vector=ref_sch_vector, month=12, day=22, sensanth=2, nday=7, dtsim=200, albroad=0.20,
...
problem = {
    ...
    'dists':['lognorm', 'lognorm','lognorm', 'lognorm', 'lognorm', 'norm',
              'unif', 'norm', 'norm', 'norm',
              'norm', 'norm', 'norm', 'norm',
              'lognorm','lognorm' ]
}
...
# sample
param_values = saltelli.sample(problem, 128) #2653
...
#Y = np.loadtxt(base_path + "txtexport\\izmir_morris.txt")
                        ...
                        #_izmir_morris_9-29
np.savetxt(base_path + "txtexport\\sobol-building_characteristics-1-17-NUMPY.txt", Y)
...
df.to_csv(base_path + "csvexport\\sobol-building_characteristics-1-17.csv")


lines = [str(Si_Temp), str(Si_CDD), str(Si_HDD)]
with open(base_path + 'txtexport\\sobol-building_characteristics-1-17.txt', 'w') as f:
    for line in lines:
        f.write(line)
        f.write('\n')
        f.write('------------')
